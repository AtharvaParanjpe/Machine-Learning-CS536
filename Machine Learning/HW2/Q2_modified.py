...
def sequence_generator():
    ...
    if(seq[0]==0):
        value = calc_majority(seq[1:6])
    else:
        value = calc_majority(seq[6:11])
    seq.append(value)
    seq = np.array(seq)
    return seq
...
def dataset_generator(m):
    ...
    for j in range(m):
        ...
        # target.append(v)
    data = np.array(data)
    columns = ["x"+ str(i)  for i in range(16)]
    columns.append("y")
...
m = []
noiseVarCounts = []
noiseVarList = ['x11', 'x12', 'x13', 'x14', 'x15'] 

for i in range(1,70000,500):
    avgNumNoise = 0
    m.append(i)
    if(i%10001==0 or i%20001==0 or i%30001==0 ):
        print(i)
    for k in range(10):
        count = 0
        train_data = dataset_generator(i)
...
for i in range(1,40001,500):
    ...
    for k in range(20):
        count = 0
        train_data = dataset_generator(i)
        tree = makeTreeByPruningDepth(train_data, d, 0)
        elements = inorderTraversal(tree)
        for x in elements:
            if(x in noiseVarList):
                count+=1
        avgNumNoise+=count
    avgNumNoise = avgNumNoise*1.0 / 20
    noiseVarCounts.append(avgNumNoise)   
...
for i in range(1,40001,200):
    ...
    for k in range(5):
        count = 0
        train_data = dataset_generator(i)
        tree = makeTreeByPruningSamples(train_data, sample)
        elements = inorderTraversal(tree)
        for x in elements:
            if(x in noiseVarList):
                count+=1
        avgNumNoise+=count
    avgNumNoise = avgNumNoise*1.0 / 5
    noiseVarCounts.append(avgNumNoise)   
